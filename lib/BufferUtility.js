...
let tmpFolder = (process.env.TEMP ? process.env.TEMP : process.env.TMP).replace(process.platform == "win32" ? /\//g : /\\\\/g, process.platform == "win32" ? "\\" : "/");
...
tmp.BufferUtility = class BufferUtility {
  ...
  readIntLE(bytesLength, pos){
    ...
    pos = Number(pos);
    if(pos+bytesLength >= this.length) throw new Error(`Position out of bound, the buffer max position is ${this.length == 0 ? 0 : this.length-1}, got ${pos+4}`);
    let i = 0
  ...
  readFloatLE(pos){
    ...
    pos = Number(pos);
    if(pos+4 >= this.length) throw new Error(`Position out of bound, the buffer max position is ${this.length == 0 ? 0 : this.length-1}, got ${pos+4}`);
