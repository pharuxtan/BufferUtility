...
let tmpFolder = process.env.TEMP ? process.env.TEMP : process.env.TMP;
...
let tmp = {func: BufferUtility, withBufferUtilityFolder: true};
...
tmp.BufferUtility = class BufferUtility {
  ...
  constructor(buffer, isFile, encoding = "utf8", off, size){
    ...
    if(this.forceOffset < 0) forceOffset = 0;
    if(this.forceLength && this.forceLength < 0) forceLength = 0;
    ...
    if(isFile){
      ...
      } else this.fileDescriptor = fs.openSync(buffer, 'rs+');
      this.isTmp = false;
    ...
    if(typeof buffer == 'object'){
      if(Buffer.isBuffer(buffer)){
        ...
        this.fileDescriptor = fs.openSync(file, 'rs+');
      } else if(tmp.func.isBufferUtility(buffer)) {
  ...
  readByte(pos){
    ...
    if(pos >= this.length){
      if(typeof pos != "number" && typeof pos != "bigint") this.position -= 2;
      throw new Error(`Position out of bound, the buffer length is ${this.length == 0 ? 0 : this.length-1}, got ${pos}`);
    }
  ...
  rightShift(length, pos, logical=false){
    ...
    if(typeof pos != "number" && typeof pos != "bigint") pos = this.position;
    pos = Number(pos);
    if(this.length - length < 0) length -= (this.length - length) * -1;
    let len = this.length - pos;
    if(len == NaN || len < 0) return this;
    for(let i = 0; i < len; i++){
      buf.writeByte(buf[len-(i+1)], len+length-(i+1));
    } for(let i = 0; i < length; i++){
      buf.writeByte(logical ? 255 : 0, pos+i);
    }
    return this;
  }
  ...
  leftShift(length, pos){
    ...
    if(this.length - length < 0) length -= (this.length - length) * -1;
    let len = this.length - pos - length;
    if(len == NaN) return this;
    if(len < 0){
      if(this.length > pos) fs.ftruncateSync(this.fileDescriptor, this.length - (length + len));
      return this;
    } for(let i = 0; i < len; i++){
      buf.writeByte(buf[pos+length+i], pos+i);
    }
    return this;
  }
  ...
  read(length, pos){
    ...
    return tmp.func(this.filename, true, undefined, Number(pos), Number(length));
  }
  ...
  writeBytes(buffer, pos){
    if(this.isDeleted) return undefined;
    if(!(Buffer.isBuffer(buffer) || tmp.func.isBufferUtility(buffer) || Array.isArray(buffer) || util.types.isUint8Array(buffer))) return;
  ...
  readIntLE(bytesLength, pos){
    ...
    pos = Number(pos);
    if(pos+bytesLength >= this.length) throw new Error(`Position out of bound, the buffer length is ${this.length == 0 ? 0 : this.length-1}, got ${pos+4}`);
    let i = 0
  ...
  write7BitEncodedInt(number, pos){
    ...
    let i = 0;
    while(number >= 0x80){
      this.WriteByte((number | 0x80) & 0xff, pos+i);
      number >>= 7;
      i++;
    }
    this.WriteByte(number & 0x7f, pos+i);
    if(addPos) this.position += i;
    return this
  }
  ...
  writeBoolean(bool, pos){
    ...
    if(!bool) bool = 0;
    if(bool > 0) bool = 1;
    return this.writeByte(number != 0, Number(pos));
  }
  ...
  readFloatLE(pos){
    ...
    pos = Number(pos);
    if(pos+4 >= this.length) throw new Error(`Position out of bound, the buffer length is ${this.length == 0 ? 0 : this.length-1}, got ${pos+4}`);
  ...
  writeChars(chars, encoding="utf8", pos){
    ...
    for(let i in array) if(typeof chars[i] != "string") throw new Error(`char at index ${i} might be a string, got ${typeof chars[i]}`);
  ...
  get length(){
    ...
    if(this.forceLength != undefined && this.forceLength != NaN && this.forceLength < stats.size) return this.forceLength;
    return stats.size;
  }

  toString(encoding){ return this.readString(this.length, encoding, 0) }

  toJSON(){ //WARNING this function can lead to a javascript fatal error for big file
    let len = this.length;
    if(len > 0){
      const data = new Array(len); //RangeError if more than 4294967295
      for(let i = 0; i < len; i++)
        data[i] = this.readByte(i);
      return { type: "BufferUtility", data }
    }
    return {type: "BufferUtility", data: [] }
  }
  ...
  toBuffer(){
    ...
    if(this.length > buffer.constants.MAX_LENGTH) throw new Error(`The length of this BufferUtility is greater than ${buffer.constants.MAX_LENGTH}, can't convert to a Buffer`);
    let buf = Buffer.allocUnsafe(this.length);
    for(let i = 0; i < this.length; i++) buf[i] = this.readByte(i);
    return buf;
  }

  toBufferList(){
    if(this.isDeleted) return [Buffer.alloc(0)];
    if(this.length <= buffer.constants.MAX_LENGTH) return [this.toBuffer()];
    ...
    do {
      ...
      for(let i = 0; i < len; i++) buf[i] = this.readByte(offset+i);
      offset += len;
      length -= len;
      arr.push(buf);
    } while(length > 0);
...
function BufferUtility(...args){
  ...
  let handler = {
      ...
      if(prop === Symbol.iterator){
        return function* () {
          for(let i = 0; i < target.length; i++){
            yield buf.readByte(i);
          }
        }
      }
      if(prop.constructor.name == "Symbol") return target[prop];
      if(!isNaN(prop)){
        let num = Number(prop);
    ...
    set (target, prop, value){
      if(["fileDescriptor", "isTmp", "forceOffset", "forceLength", "isDeleted"].includes(prop)) return value;
      if(prop == "filename"){
        if(value == "delete"){
      ...
      if(!isNaN(prop)){
        let num = Number(prop);
        return buf.writeByte(value, num);
      }
      return target[prop] = value;
    },
    ownKeys () {
      return ["fileDescriptor", "filename", "position", "isDeleted"];
    }
  }
...
BufferUtility.changeTmpFolder = function changeTmpFolder(folder, withBufferUtilityFolder = true){
  if(fs.existsSync(folder)){
    ...
    if(stats.isDirectory()){
      ...
      return tmpFolder = folder.replace(process.platform == "win32" ? "/" : "\\", process.platform == "win32" ? "\\" : "/");
    }
...
BufferUtility.concat = function concat(bufarr, createNewBU = false){
  ...
  for(let i = 0; i < bufarr.length; i++){
      if(!(Buffer.isBuffer(bufarr[i]) || tmp.func.isBufferUtility(bufarr[i]) || Array.isArray(bufarr[i]) || util.types.isUint8Array(bufarr[i]))) throw new Error(`The index ${i} of the array is not a BufferUtility/Buffer/Array/Uint8Array`);
