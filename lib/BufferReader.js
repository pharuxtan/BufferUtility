...
class BufferReader {
  ...
  ReadBytes(length, retUInt8Array){
    if(this.#close) return null;
    let bytes = [];
    for(let i=0; i<length; i++) bytes.push(this.ReadByte());
    if(retUInt8Array){
      let array = new Uint8Array(bytes.length);
      for(let i in bytes) array[i] = bytes[i];
      return array;
    } else return Buffer.from(bytes);
  }
  ...
  ReadString(length, encoding="utf8"){
    ...
    switch(encoding.toLowerCase()){
      ...
      case 'latin1':
      case 'binary':
        string = latin1Slice(this.ReadBytes(length));
        break;
      case 'base64':
